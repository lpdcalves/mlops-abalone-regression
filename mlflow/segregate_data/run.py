...
def process_args(args):
    ...
    run = wandb.init(job_type="segregate_data")

    logger.info("Downloading artifact")
    artifact = run.use_artifact(args.input_artifact)
    artifact_path = artifact.file()

    clean_abalone = pd.read_csv(artifact_path)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for split, clean_abalone in splits.items():
            ...
            clean_abalone.to_csv(temp_path,index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split a dataset into train and test and send it to W&B",
